...
from common import activate_venv_path, venv_ready
...
try:
    ...
    print("[ERROR] Missing Python dependencies (scapy).")
    if venv_ready():
...
LINUX_VIRTUAL_KEYWORDS = (
    "docker", "veth", "virbr", "br-", "tun", "tap", "lo", "vmnet",
    "vboxnet", "wg", "tailscale", "dummy", "cni", "flannel", "calico",
    "ifb", "erspan", "gre", "sit", "ip6tnl", "ip6gre",
)
        ...
        if alias:
            for iface in scapy.get_if_list():
                if alias.lower() in iface.lower() or iface.lower() in alias.lower():
                    conf.iface = iface
                    return iface
            if os.name != "nt" and alias in scapy.get_if_list():
                conf.iface = alias
                return alias
...
def _is_linux_virtual_iface(name: str) -> bool:
    low = name.lower()
    if low == "lo":
        return True
    return any(low.startswith(k) or k in low for k in LINUX_VIRTUAL_KEYWORDS)


def _linux_iface_is_wifi(name: str) -> bool:
    wireless = f"/sys/class/net/{name}/wireless"
    return os.path.isdir(wireless)


def _linux_iface_gateway(name: str) -> str:
    ok, out = run_cmd(f"ip -4 route show dev {name} default")
    if ok:
...
def _get_linux_network_adapters(include_down: bool = False) -> list[NetworkAdapter]:
    ...
    for entry in addrs:
        ...
        for info in entry.get("addr_info", []):
            if info.get("family") != "inet":
                continue
            ip = str(info.get("local", "") or "").strip()
    ...
    for link in links:
        ...
        if not name or _is_linux_virtual_iface(name):
            continue
        operstate = str(link.get("operstate", "UNKNOWN") or "UNKNOWN").lower()
        if not include_down and operstate not in ("up", "unknown"):
            continue
        mac = str(link.get("address", "") or "").replace("-", ":").lower()
        ip, prefix = addr_by_iface.get(name, ("", 24))
        gateway = _linux_iface_gateway(name)
        adapter_type = "WiFi" if _linux_iface_is_wifi(name) else "Ethernet"
        desc = f"Linux {adapter_type} ({name})"
        adapters.append(
            NetworkAdapter(
                index=idx,
                name=name,
...
def request_admin_elevation() -> None:
    ...
    if os.name != "nt":
        safe_print(f"{Fore.YELLOW}[*] Root privileges required.")
        safe_print(f"{Fore.YELLOW}[*] Re-launching with sudo...")
        script_path = os.path.abspath(sys.argv[0])
        try:
            os.execvp(
                "sudo",
                ["sudo", sys.executable, script_path] + sys.argv[1:],
            )
        except OSError as exc:
            safe_print(f"{Fore.RED}[-] sudo elevation failed: {exc}")
            safe_print(f"{Fore.YELLOW}[!] Run with: sudo {sys.executable} {script_path}")
            sys.exit(1)
        sys.exit(0)
...
class NetworkResilience:
    ...
    def capture_initial_wifi(self):
        if os.name == "nt":
            ...
            ok, out = run_cmd("nmcli -t -f DEVICE,TYPE,STATE device status")
            if ok:
                for line in out.splitlines():
                    parts = line.split(":")
                    if len(parts) >= 3 and parts[1] == "wifi" and parts[2] == "connected":
                        self.initial_ctx.interface_name = parts[0]
                        self.initial_ctx.is_wifi = True
                        break
            if self.initial_ctx.interface_name:
                ok, out = run_cmd(
                    f"nmcli -t -f GENERAL.CONNECTION dev show {self.initial_ctx.interface_name}"
                )
                if ok:
                    for line in out.splitlines():
            ...
            if not self.initial_ctx.ssid:
                ok, out = run_cmd("iwgetid -r")
                if ok and out.strip():
                    self.initial_ctx.ssid = out.strip()
                    self.initial_ctx.is_wifi = True
    ...
    def _warn_location_required(self):
        ...
        if os.name == "nt":
            ...
            log_warn("Ensure NetworkManager (nmcli) or iw/iwlist is installed and WiFi is enabled.")
            log_warn("WiFi scan requires root privileges on most Linux distributions.")

    def _linux_wifi_iface(self) -> str:
        if self.ctx.interface_name and _linux_iface_is_wifi(self.ctx.interface_name):
            return self.ctx.interface_name
        for adapter in _get_linux_network_adapters():
            if adapter.adapter_type == "WiFi":
                return adapter.name
        ok, out = run_cmd("nmcli -t -f DEVICE,TYPE device status")
        if ok:
            for line in out.splitlines():
                parts = line.split(":")
    ...
    def _scan_wifi_linux(self) -> list[WifiNetwork]:
        ...
        if ok and out.strip():
            for line in out.splitlines():
                ...
                auth = ":".join(parts[3:]).strip() or "Unknown"
                ...
                networks.append(
                    WifiNetwork(
                        ...
                        signal=signal,
                        auth=auth,
                        open_network=auth.lower() in ("", "--", "open", "none"),
                    )
                )
        ...
        if not networks:
            ...
            if iface:
                run_cmd(f"iw dev {iface} scan trigger 2>/dev/null")
                time.sleep(2)
                ok, out = run_cmd(f"iw dev {iface} scan dump 2>/dev/null")
                if ok and out.strip():
                    current_ssid = ""
                    current_bssid = ""
                    current_signal = 0
                    for line in out.splitlines():
                        ...
                        if ssid_m:
                            ...
                            continue
                        bssid_m = re.search(r"BSS ([0-9a-f:]+)", line, re.I)
                        if bssid_m:
                            current_bssid = bssid_m.group(1).lower()
                            continue
                        sig_m = re.search(r"signal:\s*(-?\d+)", line, re.I)
                        if sig_m:
                            try:
                                dbm = int(sig_m.group(1))
                                current_signal = max(0, min(100, 2 * (dbm + 100)))
                            except ValueError:
                                current_signal = 0
                            continue
                        if current_ssid and current_bssid:
                            key = (current_ssid, current_bssid)
                            if key not in seen:
                                seen.add(key)
                                networks.append(
                                    WifiNetwork(
        ...
        if not networks:
            ok, out = run_cmd("iwlist scan 2>/dev/null")
            if ok and out.strip():
                current_ssid = ""
                current_bssid = ""
                current_signal = 0
                ...
                for line in out.splitlines():
                    ...
                    if essid_m:
                        current_ssid = essid_m.group(1)
                        continue
                    addr_m = re.search(r"Address:\s*([0-9A-F:]+)", line, re.I)
                    if addr_m:
                        current_bssid = addr_m.group(1).lower()
                        continue
                    qual_m = re.search(r"Signal level[=:](-?\d+)", line, re.I)
                    if qual_m:
                        try:
                            dbm = int(qual_m.group(1))
                            current_signal = max(0, min(100, 2 * (dbm + 100)))
                        except ValueError:
                            current_signal = 0
                        continue
                    enc_m = re.search(r"Encryption key:(on|off)", line, re.I)
                    if enc_m:
                        current_auth = "Open" if enc_m.group(1).lower() == "off" else "Secured"
                        if current_ssid and current_bssid:
                            key = (current_ssid, current_bssid)
                            if key not in seen:
                                seen.add(key)
                                networks.append(
                                    WifiNetwork(
                                        ...
                                        signal=current_signal,
                                        auth=current_auth,
                                        open_network=current_auth.lower() == "open",
                                    )
                                )
                        current_ssid = ""
                        current_bssid = ""
    ...
    def _method_dhcp_renew(self) -> bool:
        ...
        if os.name == "nt":
            ...
            if iface:
                ok, _ = run_cmd(f"nmcli device reapply {iface} 2>/dev/null")
                if ok:
                    time.sleep(3)
                    return self.has_internet()
                run_cmd(f"dhclient -r {iface} 2>/dev/null")
                run_cmd(f"dhclient {iface} 2>/dev/null")
            else:
                run_cmd("dhclient -r 2>/dev/null")
                run_cmd("dhclient 2>/dev/null")
        return self.has_internet()
    ...
    def _method_change_local_ip(self) -> bool:
        ...
        if os.name == "nt":
            ...
            ok, _ = run_cmd(
                ...
                f"ip addr add {new_ip}/24 dev {iface} && "
    ...
    def _rank_wifi_candidates(self, networks: list[WifiNetwork]) -> list[WifiNetwork]:
        ...
        if self.initial_ctx.is_wifi:
            if os.name == "nt":
                ...
                bssid = ""
                if iface:
                    ok, out = run_cmd(f"iw dev {iface} link 2>/dev/null")
                    if ok:
                        match = re.search(r"Connected to ([0-9a-f:]+)", out, re.I)
                        if match:
                            bssid = match.group(1)
            if bssid:
    ...
    def _connect_wifi(self, ssid: str, password: str, auth: str = "") -> bool:
        ...
        ssid_q = ssid.replace("'", "'\\''")
        if password:
            cmd = f"nmcli dev wifi connect '{ssid_q}' password '{password}'"
        else:
            cmd = f"nmcli dev wifi connect '{ssid_q}'"
        ok, out = run_cmd(cmd)
        if ok or "successfully activated" in out.lower():
            time.sleep(4)
            return True
        return False
...
<WLANProfile xmlns="http://www.microsoft.com/networking/WLAN/profile/v1">
    ...
    def _detect_network(self, interface_name: str = "") -> tuple[str, str, str]:
        ...
        if os.name == "nt" and iface:
            ...
            ok, out = run_cmd(f"ip -j -4 addr show dev {iface}")
            if ok and out.strip():
                try:
                    ...
                    for entry in entries:
                        for info in entry.get("addr_info", []):
                            if info.get("family") != "inet":
                                continue
                            ip_address = str(info.get("local", "") or "")
                            if not ip_address or ip_address.startswith("169.254."):
                                continue
                            prefix = int(info.get("prefixlen", 24) or 24)
                            gateway = _linux_iface_gateway(iface)
                            ip_range = str(ipaddress.ip_interface(f"{ip_address}/{prefix}").network)
                            return ip_address, gateway, ip_range
        ...
        if os.name == "nt" and not gateway:
            ...
            if iface:
                gateway = _linux_iface_gateway(iface)
            if not gateway:
                ok, output = run_cmd("ip -4 route show default")
                if ok:
        ...
        if ip_address and ip_address != "127.0.0.1":
            if iface:
                for adapter in _get_linux_network_adapters():
                    if adapter.name == iface and adapter.ip == ip_address and adapter.prefix:
                        try:
                            ip_range = str(
                                ipaddress.ip_interface(f"{ip_address}/{adapter.prefix}").network
                            )
                            break
                        except ValueError:
                            pass
    ...
    def _get_active_interface(self) -> str:
        if self.ctx.interface_name:
            return self.ctx.interface_name
        if os.name != "nt":
            ok, out = run_cmd("ip -j -4 route show default")
            if ok and out.strip():
                try:
                    routes = json.loads(out)
                    if routes:
                        return str(routes[0].get("dev", "") or "")
                except json.JSONDecodeError:
                    pass
            ok, out = run_cmd("ip -4 route show default")
            if ok:
                ...
                if match:
                    return match.group(1)
            for adapter in _get_linux_network_adapters():
                if adapter.ip and adapter.gateway:
                    return adapter.name
            return ""
        ok, out = run_cmd(
            'powershell -NoProfile -Command '
    ...
    def _get_wifi_password(self, ssid: str) -> str:
        if os.name != "nt":
            ok, out = run_cmd(
                f"nmcli -s -g 802-11-wireless-security.psk connection show '{ssid}' 2>/dev/null"
            )
            if ok and out.strip() and out.strip() != "--":
                return out.strip()
            ok, out = run_cmd(
                f"nmcli -s -g wifi-sec.psk connection show '{ssid}' 2>/dev/null"
            )
            if ok and out.strip() and out.strip() != "--":
                return out.strip()
            return ""
...
{Fore.LIGHTMAGENTA_EX}NOTES:
  ...
  - Windows: requests administrator elevation via UAC.
